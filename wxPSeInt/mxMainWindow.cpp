...
void mxMainWindow::OnFileOpen(wxCommandEvent &evt) {
	...
	dlg.SetWildcard(_Z("Todos los archivos|*|Algoritmos en pseudocÃ³digo|*.psc;*.PSC|Ejercicios empaquetados|*.psz;*.PSZ;*.psx;*.PSX|Archivos de texto|*.txt;*.TXT"));
	if (dlg.ShowModal() == wxID_OK) {
		wxArrayString paths;
		dlg.GetPaths(paths);
//------------------------------------------------------------------------------------------------------------
...
void mxMainWindow::SelectError(wxString text) {
	// elegir el fuente que genera al error
	int index = notebook->GetPageIndex(last_source);
	if (index==wxNOT_FOUND) return;
	notebook->SetSelection(index);
	// seleccionar esa linea e instruccion
	long l,i=-1;
...
void mxMainWindow::InsertCode(wxArrayString &toins) {
	IF_THERE_IS_SOURCE {
		...
		// ver si habï¿½a algo en la misma linea despues de la seleccion para saber si va un enter mï¿½s o no
		...
		// ver si habï¿½a algo en la misma linea antes de la seleccion para saber si va un enter mï¿½s o no
		...
		// insertar el cï¿½digo con su correspondiente formato (en la linea dada por line, que estï¿½ en blanco)
		int oline=line;
		...
		// corregir el indentado y seleccionar el cï¿½digo nuevo 
...
void mxMainWindow::OnFileEditFlow (wxCommandEvent & evt) {
	IF_THERE_IS_SOURCE {
		...
		if (source->GetFlowSocket()) { 
			...
			wxMessageBox(_Z("Su algoritmo contiene comentarios. Si edita el diagrama y guarda los cambios perder los comentarios!"),_Z("Advertencia"),wxOK|wxICON_EXCLAMATION,this);
		}
		wxString fname=source->SaveTemp();
		if (debug->debugging)
			debug->Stop();
...
void mxMainWindow::ShowVarsPanel (bool show, bool anim) {
	...
	if (show) CheckIfNeedsRTS(); // aunque el codigo ya estï¿½ analizado, el arbol estï¿½ vacio porque si no se muestra no se actualiza
